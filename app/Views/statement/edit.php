...
<form action="<?= site_url('/ctrlstatement/update/' . $datast['id']) ?>" method="post" enctype="multipart/form-data">
<input type="hidden" name="id" value="<?= $datast['id'] ?>">
  <div class="mb-3">
        <label for="foto" class="form-label">Foto Barang</label>
        <input type="file" class="form-control" id="foto" name="foto">
        <?php if ($datast['foto']): ?>
            <img src="<?= base_url('upload/' . $datast['foto']) ?>" width="150">
        <?php endif; ?>
    </div>
    </div>
</div>
