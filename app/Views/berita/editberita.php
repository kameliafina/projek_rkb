...
<form action="<?= site_url('/ctrlberita/update/' . $databerita['id']) ?>" method="post" enctype="multipart/form-data">
  ...
  <div class="row mb-3">
    ...
    <select class="form-control" id="kategori_id" name="kategori_id">
            ...
            <?php foreach ($kategori as $kat): ?>
                <option value="<?= $kat['id'] ?>" <?= (old('kategori_id', $databerita['kategori_id']) == $kat['id']) ? 'selected' : '' ?>>
                    <?= $kat['nama_kategori_b'] ?>
                </option>
            <?php endforeach; ?>
        </select>
